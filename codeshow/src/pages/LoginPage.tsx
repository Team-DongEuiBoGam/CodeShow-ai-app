...
interface Props {
    onDone: () => void
    onGoToRegister: () => void // 회원가입 페이지 이동 함수 추가 필요
}
...
export default function LoginPage({ onDone, onGoToRegister }: Props) {
    ...
    const handleLogin = (e?: React.FormEvent) => {
        if (e) e.preventDefault() // 엔터 키 새로고침 방지
            ...
            <div style={{
                ...
                {view === 'choice' ? (
                    ...
                    <form onSubmit={handleLogin} style={{ display: 'flex', flexDirection: 'column' }}>
                        ...
                        <button type="submit" style={{ width: '100%', padding: '14px', background: '#6c8cff', border: 'none', borderRadius: '12px', fontSize: '15px', fontWeight: 600, color: '#fff', cursor: 'pointer', boxSizing: 'border-box' }}>로그인</button>
                        <button type="button" onClick={onGoToRegister} style={{ background: 'none', border: 'none', color: '#6c8cff', fontSize: '13px', cursor: 'pointer', marginTop: '12px', textDecoration: 'underline' }}>회원가입 하기</button>
